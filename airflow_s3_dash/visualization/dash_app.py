...
def get_data_from_minio():
    try:
        ...
        # Đọc dữ liệu từ object vào DataFrame
        df = pd.read_csv(obj['Body'])

        df = df[df.percentTotalVolume > 0.5]

        return df

    except Exception as e:
        print(f"Error reading file from MinIO: {str(e)}")
        return None
